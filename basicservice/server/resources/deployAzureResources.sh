...
deploy_template() {
    ...
    az deployment sub create --name "${BASE_FILENAME}-${RESOURCES_NAME}-basicservice-deploy" --location eastus --template-file "$TEMPL_FILE" --parameters ServiceResources.Parameters.json -o json > $TEMPL_DIR/.${BASE_FILENAME}_tmp.json
