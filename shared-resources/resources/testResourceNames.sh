...
if [[ $clusterName =~ ^\[parameters\(.*\)\]$ ]]; then
    ...
    if [[ $parameterValue =~ ^format\(.*\)$ ]]; then
        ...
        # Example: format('servicehubval-{0}-cluster', parameters('resourcesName')) -> servicehubval-<value of resourcesName>-cluster
        # Value is extracted from scope bindings for example -> servicehubval-official-cluster
